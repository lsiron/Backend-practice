...
app.get('/detail/:id', async (req, res) => {   //:id/:id2/:id3 처럼 URL 파라미터를 여러개 넣어도 상관없음. : 뒤엔 아무거나 변수명 입력하면 됨
  try{
    let result = await db.collection('post')        // 외우기, mongoDB 데이터 하나를 가져오는 방법 
...
app.put('/edit', async (req, res) => {   // method-override 설치 후, 해당 API에 연결되는 form 태그 action칸 작업 하고나서 put요청  
  try{
    ...
    await db.collection('post').updateOne({_id : new ObjectId(req.body.id) },  // db에 있는 doc를 하나 수정할땐 updateOne()을 씀
...
app.get('/list/:id', async (req, res) => {     
  ...
  .skip((req.params .id - 1)*5).limit(5).toArray() // skip(10) 뜻은 데이터를 맨 위에서 10개를 skip한다는 뜻, limit(5) 뜻은 데이터를 맨 위에서 5개만 가져온다는 뜻   
